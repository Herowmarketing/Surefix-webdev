  ...
  return (
    <div className="min-h-screen bg-[#0d1117]">
      <section className="mx-auto max-w-3xl px-5 py-32 text-center lg:px-8">
        ...
        <p className="mb-7 text-base text-white/65" style={{ fontFamily: 'Georgia, serif' }}>
...
function LocationView({ loc }: { loc: LocationKey }) {
  ...
  return (
    <div className="min-h-screen bg-[#0d1117]">
        ...
        <div className="relative mx-auto max-w-7xl px-5 pb-14 pt-[max(8.5rem,calc(7rem+env(safe-area-inset-top,0px)))] sm:pb-20 sm:pt-[max(10rem,calc(8.5rem+env(safe-area-inset-top,0px)))] lg:px-8 lg:pb-20">
          <motion.div initial="hidden" animate="visible" variants={stagger} className="max-w-3xl">
            <motion.div variants={fadeUp} custom={0}>
              <Link href="/locations">
                <span
                  className="mb-5 inline-flex min-h-[40px] cursor-pointer items-center gap-2 text-[11px] font-bold uppercase tracking-[0.32em] text-white/55 transition-colors hover:text-white"
                  style={{ fontFamily: SANS }}
                >
      ...
      <section className="border-y border-white/[0.05] bg-[rgba(57,70,150,0.05)]">
          ...
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-2">
            ...
            >
              ...
              <motion.div variants={fadeUp} custom={2} className="flex flex-wrap gap-2">
                {loc.highlights.map((n) => (
                  <span
                    key={n}
                    className="rounded-full border border-white/[0.1] px-3 py-1.5 text-xs font-semibold text-white/75"
                    style={{ background: 'rgba(255,255,255,0.03)', fontFamily: SANS }}
                  >
                    {n}
                  </span>
                ))}
            ...
            >
              <motion.div variants={fadeUp} custom={0} className="mb-4 flex items-center gap-2">
                ...
                <p
                  className="text-[10px] font-bold uppercase tracking-[0.32em] text-white/55"
                  style={{ fontFamily: SANS }}
                >
        ...
        >
          {loc.featuredServices.map((svc, i) => (
            <motion.div
              ...
              variants={fadeUp}
              custom={i}
              className="flex h-full flex-col gap-3 rounded-2xl border border-white/[0.08] p-5"
              style={{ background: 'rgba(255,255,255,0.025)' }}
            >
              <Sparkles size={18} className="text-[#394696]" />
              <p className="text-base font-bold text-white" style={{ fontFamily: SANS }}>
                {svc}
              </p>
              <p className="text-sm text-white/55" style={{ fontFamily: 'Georgia, serif' }}>
      ...
      {/* NEARBY LOCATIONS */}
      <section className="border-t border-white/[0.05] bg-[rgba(57,70,150,0.05)]">
        <div className="mx-auto max-w-7xl px-5 py-14 sm:py-20 lg:px-8">
          <p
            className="mb-3 text-[10px] font-bold uppercase tracking-[0.4em] text-[#394696]"
            style={{ fontFamily: SANS }}
          ...
          <div className="flex flex-wrap gap-2">
            {otherLocations.map((l) => (
              <Link key={l.slug} href={`/locations/${l.slug}`}>
                <span
                  className="inline-flex min-h-[40px] cursor-pointer items-center gap-2 rounded-full border border-white/[0.1] px-4 py-2 text-xs font-semibold text-white/80 transition-colors hover:border-[#394696]/60 hover:bg-[#394696]/15 hover:text-white"
                  style={{ background: 'rgba(255,255,255,0.03)', fontFamily: SANS }}
                >
      ...
      <section className="mx-auto max-w-4xl px-5 py-16 text-center sm:py-20 lg:px-8">
        ...
        >
          ...
          <p
            className="mx-auto mb-7 max-w-2xl text-base leading-relaxed text-white/55"
            style={{ fontFamily: 'Georgia, serif' }}
          >
          ...
          <div className="flex flex-wrap justify-center gap-3">
            ...
            <Link href="/contact">
              <span
                className="inline-flex min-h-[48px] cursor-pointer items-center gap-2 rounded-full border border-white/15 px-7 py-3.5 text-[11px] font-bold uppercase tracking-[0.22em] text-white/85 transition-colors hover:bg-white/[0.06]"
                style={{ fontFamily: SANS }}
              >
                Contact our team
              </span>
